...
class TorchPOCAOptimizer(TorchOptimizer):
    ...
    def _validate_enhanced_oracle_navigation_contract(self) -> None:
        network_settings = self.trainer_settings.network_settings
        if network_settings.memory is not None:
            raise UnityTrainerException(
                "Enhanced Oracle navigation mode requires memory=null"
            )
        if self._visual_checkpoint is not None or self._use_navigation_probe_features:
            raise UnityTrainerException(
                "Enhanced Oracle navigation cannot load a visual encoder or navigation probe"
            )
        observation_contract = [
            (spec.name, spec.shape)
            for spec in self.policy.behavior_spec.observation_specs
        ...
        expected_contract = [
            ("01_EnhancedOracleNavigation", (212,)),
            ("VectorSensor_size13", (13,)),
        ]
        if observation_contract != expected_contract:
            raise UnityTrainerException(
                "Enhanced Oracle navigation observation contract mismatch: "
                f"expected {expected_contract}, got {observation_contract}"
            )
        action_spec = self.policy.behavior_spec.action_spec
        if action_spec.continuous_size != 2 or action_spec.discrete_size != 0:
            raise UnityTrainerException(
                "Enhanced Oracle navigation requires exactly two continuous actions "
                "and no discrete actions"
            )
        logger.info(
            "Enhanced Oracle navigation audit: navigation_sensor=212; "
            "navigation_padding=300; direct_geometry=96; clearance=16; path=88; "
            "velocity=4; teammate_motion=8; manual_vector=13; camera_sensors=0; "
            "ray_sensors=0; continuous_actions=2; trainable_navigation_parameters=0; "
            "lstm=false"
        )
